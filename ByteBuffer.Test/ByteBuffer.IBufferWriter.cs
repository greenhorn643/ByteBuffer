...
[TestClass]
public class ByteBuffer_IBufferWriter
{
	[TestMethod]
	[DataRow(100000, 10, 20)]
	[DataRow(10000000, 1000000, 2000000)]
	public void FunctionsAsBufferWriter_GetBuffer_PopRange(int nItems, int maxWrite, int maxRead)
	{
		var rng = new Random();
		...
		while (readIdx < nItems)
		{
			...
			var mem = buffer.GetMemory(nToWrite);
			bytesToFeed.AsSpan(writeIdx, nToWrite).CopyTo(mem.Span);
			buffer.Advance(nToWrite);
			writeIdx += nToWrite;
	...
	[DataRow(100000, 10, 20)]
	[DataRow(10000000, 1000000, 2000000)]
	public void FunctionsAsBufferWriter_GetBuffer_PeekRange(int nItems, int maxWrite, int maxRead)
	{
		var rng = new Random();
		var bytesToFeed = new byte[nItems];
		rng.NextBytes(bytesToFeed);
